...
func main() {
	...
	if result.Exists() {
		for i := 0; i < result.Count(); i++ {
			nameResult := result.Index(i).Get("name")
			priceResult := result.Index(i).Get("price")

			name, err := nameResult.String()
			if err != nil {
				fmt.Printf("   Error getting name: %v\n", err)
				continue
			}

			price, err := priceResult.Float()
			if err != nil {
	...
	if result.Exists() {
		for i := 0; i < result.Count(); i++ {
			name := result.Index(i).Get("name").MustString()
			fmt.Printf("   - %s\n", name)
		}
	}
	fmt.Println()
	...
	if result.Exists() {
		for i := 0; i < result.Count(); i++ {
			name := result.Index(i).Get("name").MustString()
			price := result.Index(i).Get("price").MustFloat()
			fmt.Printf("   - %s: $%.2f\n", name, price)
		}
	}
	fmt.Println()
	...
	if result.Exists() {
		for i := 0; i < result.Count(); i++ {
			name := result.Index(i).Get("name").MustString()
			price := result.Index(i).Get("price").MustFloat()
			category := result.Index(i).Get("category").MustString()
			fmt.Printf("   - %s: $%.2f (%s)\n", name, price, category)
		}
	}
	fmt.Println()
	...
	if result.Exists() {
		for i := 0; i < result.Count(); i++ {
			name := result.Index(i).Get("name").MustString()
			age := result.Index(i).Get("age").MustInt()
			fmt.Printf("   - %s (age %d)\n", name, age)
		}
	}
}
